...
export default function App() {
  ...
  // ==========================================
  // 4. حالات نظام الرفع والأتمتة السحابية (محدثة)
  // ==========================================
  ...
  const fetchDriveList = async () => {
    ...
    try {
      ...
      console.error(e);
      alert("فشل جلب الملفات من جوجل درايف.");
    }
  ...
  const handleServerProcess = async (e) => {
    ...
    if ((inputType === 'local' || inputType === 'drive') && !localFile) return alert('اختر ملفاً من جهازك أو من جوجل درايف أولاً!');
      ...
      {currentView === 'automation' ? (
        <main className="container mx-auto p-4 mt-6 max-w-4xl animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className={`rounded-3xl p-6 md:p-8 border shadow-sm ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            {/* التبويبات الثلاثة (جهاز - درايف - رابط) */}
            <div className="flex flex-col sm:flex-row mb-8 bg-slate-100 dark:bg-slate-700 p-1 rounded-xl gap-1">
              <button 
                ...
                className={`flex-1 py-3 px-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${inputType === 'local' ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}`}
              >
                <FileAudio size={18} /> الجهاز
              </button>
              <button 
                onClick={() => { setInputType('drive'); setServerResult(null); }}
                className={`flex-1 py-3 px-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${inputType === 'drive' ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}`}
              >
                <HardDrive size={18} /> Google Drive
              </button>
              <button 
                onClick={() => { setInputType('url'); setServerResult(null); }}
                className={`flex-1 py-3 px-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${inputType === 'url' ? 'bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}`}
              >
                <LinkIcon size={18} /> يوتيوب
              </button>
            </div>
            ...
            <form onSubmit={handleServerProcess} className="space-y-6">
              ...
              {inputType === 'url' ? (
                <div className="animate-in fade-in">
                  ...
                  <div className={`mt-3 p-3 text-xs rounded-xl flex items-start gap-2 ${darkMode ? 'bg-blue-900/20 text-blue-300' : 'bg-blue-50 text-blue-800'}`}>
                    ...
                    <span>سيرفرات Hugging Face قد تقوم بحظر روابط يوتيوب أو تليجرام أحياناً بسبب الضغط. إذا واجهت خطأ، يُفضل استخدام الرفع من الجهاز أو درايف.</span>
                  </div>
                </div>
              ) : inputType === 'local' ? (
                <div className="animate-in fade-in">
                  ...
                  <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition ${darkMode ? 'border-slate-600 bg-slate-700/30' : 'border-slate-300 bg-slate-50 hover:bg-slate-100'}`}>
                    ...
                    <label htmlFor="local-upload" className="cursor-pointer flex flex-col items-center">
                      <FileAudio size={48} className={`mb-4 ${localFile && !localFile.isDrive ? 'text-green-500' : (darkMode ? 'text-slate-500' : 'text-slate-400')}`} />
                      <span className={`font-medium text-lg ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                        {localFile && !localFile.isDrive ? localFile.name : 'اضغط هنا لاختيار ملف من جهازك'}
                      </span>
                    </label>
                  </div>
                </div>
                ...
                <div className="animate-in fade-in">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {!driveToken && (
                       <button type="button" onClick={connectDrive} className="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:underline">
                         ربط درايف الآن
                       </button>
                    )}
                  </div>
                  ...
                  {!driveToken ? (
                    ...
                    <div className={`border rounded-2xl p-4 ${darkMode ? 'border-slate-600 bg-slate-700/30' : 'border-slate-300 bg-slate-50'}`}>
                       ...
                       {isLoadingDrive ? (
                         ...
                         <p className="text-center py-6 text-sm text-slate-500">لا توجد ملفات صوتية/فيديو حديثة. اضغط تحديث.</p>
                       )}
                    </div>
                  )}
                  {localFile && localFile.isDrive && (
                    <p className="mt-3 text-sm text-green-600 dark:text-green-400 font-bold text-center">
                      <CheckCircle size={16} className="inline mr-1" /> تم استيراد ({localFile.name}) وجاهز للمعالجة.
                    </p>
                  )}
                </div>
              )}
              ...
              <div className={`p-4 rounded-xl border flex items-center justify-between cursor-pointer ${autoUploadDrive ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-slate-200 dark:border-slate-700 bg-transparent'}`} onClick={() => setAutoUploadDrive(!autoUploadDrive)}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-xs ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>سيقوم المتصفح برفع الأجزاء المقطوعة لدرايف الخاص بك باستخدام باقتك.</p>
                  </div>
                </div>
              ...
              <button type="submit" disabled={isProcessingServer} className={`w-full py-4 rounded-xl font-black text-lg transition flex justify-center items-center gap-3 shadow-lg ${isProcessingServer ? 'bg-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white'}`}>
                {isProcessingServer ? <><Loader2 className="animate-spin" size={24} /> جاري المعالجة (تجنب غلق الصفحة)...</> : <><UploadCloud size={24} /> بدء المعالجة {autoUploadDrive ? 'والرفع لدرايف' : ''}</>}
              </button>
            </form>
                        ...
                        <div className="flex gap-2">
                          {part.drive_link && (
                            <a href={part.drive_link} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs font-bold px-3 py-1.5 bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400 rounded-lg hover:bg-green-200 transition">
                              <UploadCloud size={14}/> مشاهدة درايف
                            </a>
                          )}
                ...
                {editingSubjectId === subject.id ? (
                  ...
                  <div className={`flex items-center justify-between transition-all rounded-xl overflow-hidden border ${
                    ...
                    <div className="flex items-center gap-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => deleteSubject(subject.id)} className={`p-1.5 rounded-md transition ${darkMode ? 'text-slate-400 hover:text-red-400 hover:bg-slate-600' : 'text-slate-400 hover:text-red-600 hover:bg-red-100'}`}><Trash size={14} /></button>
                    </div>
                  </div>
                )}
              </li>
                            ...
                            {editingLectureId === lecture.id ? (
                              ...
                              <>
                                ...
                                <div className={`flex gap-1 rounded-lg p-1 border ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
                                  ...
                                  <button onClick={() => deleteLecture(activeSubject.id, lecture.id)} className={`p-1.5 transition ${darkMode ? 'text-slate-400 hover:text-red-400' : 'text-slate-400 hover:text-red-600'}`}><Trash size={14} /></button>
                                </div>
                              </>
                            )}
                          </div>
            ...
            <div className={`h-full flex items-center justify-center rounded-2xl p-12 border min-h-[50vh] ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200 shadow-sm'}`}>
              <div className="text-center">
                <div className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 ${darkMode ? 'bg-indigo-900/30' : 'bg-indigo-50'}`}>
                  <List size={40} className={darkMode ? 'text-indigo-400' : 'text-indigo-400'} />
                </div>
